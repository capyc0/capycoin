...
{
    if (nHeight > 3000000)	return 40000 * COIN;
    if (nHeight > 2000000)	return 35000 * COIN;
    if (nHeight > 1500000)	return 30000 * COIN;
    if (nHeight > 1000000)	return 25000 * COIN;
    if (nHeight > 500000)	return 20000 * COIN;
    if (nHeight > 300000)	return 15000 * COIN;
    if (nHeight > 100000)	return 12500 * COIN;
    if (nHeight > 50000)	return 10000 * COIN;
    if (nHeight > 10000)	return 7500 * COIN;
    if (nHeight > 1000)	    return 5000 * COIN;
    if (nHeight > 1)		return 2500 * COIN;
    return 40000 * COIN;
}

CAmount CMasternode::GetBlockValue(int nHeight)
{
    if (nHeight > 3000000)	return 0.5   * COIN;
    if (nHeight > 2000000)	return 1     * COIN;
    if (nHeight > 1500000)	return 2     * COIN;
    if (nHeight > 1000000)	return 3.5   * COIN;
    if (nHeight > 500000)	return 6     * COIN;
    if (nHeight > 300000)	return 5     * COIN;
    if (nHeight > 100000)	return 4     * COIN;
    if (nHeight > 50000)	return 3     * COIN;
    if (nHeight > 10000)	return 2.5   * COIN;
    if (nHeight > 1000)	    return 1.5   * COIN;
    if (nHeight > 0)		return 250   * COIN;
    return 0 * COIN;
}
...
{
    if (nHeight > 1000) return GetBlockValue(nHeight) * 5 / 100;
    return 0;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 CAP tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 CAP tx -> 1 confirmation
