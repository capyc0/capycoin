...
const char * const PIVX_CONF_FILENAME = "cap.conf";
const char * const PIVX_PID_FILENAME = "cap.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Capybara Coin only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\cap
// Windows >= Vista: C:\Users\Username\AppData\Roaming\cap
// Mac: ~/Library/Application Support/cap
// Unix: ~/.cap
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Capybara Coin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ddrParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ddrParams
    // Mac: ~/Library/Application Support/ddrParams
    // Unix: ~/.cap-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CapybaraCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "CapybaraCoinParams";
#else
    // Unix
    return pathRet / ".cap-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cap.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Capybara Coin config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cap.conf
