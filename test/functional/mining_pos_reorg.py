...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected CAP and zDDR supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zDDRsupply']:
                assert_equal(node['zDDRsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check CAP and zDDR supply at the beginning
        # ------------------------------------------
        # zDDR supply: 2 coins for each denomination
        ...
        # CAP supply: block rewards minus burned fees for minting
        ...
        # Verify that CAP and zDDR supplies were properly updated after the spends and reorgs
        self.log.info("Check CAP and zDDR supply...")
